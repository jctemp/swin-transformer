...
@pytest.mark.parametrize(
    ...
    [
        ((224, 224), (16, 16), 3, 512, PatchMode.CONCATENATE),
        ((224, 224), (16, 16), 3, 512, PatchMode.CONVOLUTION),
        ((128, 128), (8, 8), 1, 256, PatchMode.CONCATENATE),
        ((128, 128), (8, 8), 1, 256, PatchMode.CONVOLUTION),
    ],
)
def test_patch_embedding_2d(device, input_size, patch_size, in_channels, embed_dim, patch_mode):
    x = torch.randn((2, in_channels, *input_size)).to(device)
    pe = PatchEmbedding2D(input_size, patch_size, in_channels, embed_dim, nn.LayerNorm, patch_mode).to(device)
    y = pe(x)
    ...
    assert y.device.type == device.type, f"Expected device {device}, got {y.device}"
...
@pytest.mark.parametrize(
    ...
    [
        ((4, 6), (2, 3), 12, PatchMode.CONCATENATE),
        ((4, 6), (2, 3), 12, PatchMode.CONVOLUTION),
        ((8, 8), (2, 2), 64, PatchMode.CONCATENATE),
        ((8, 8), (2, 2), 64, PatchMode.CONVOLUTION),
    ],
)
def test_patch_merging_2d(device, input_size, merge_size, embed_dim, patch_mode):
    x = torch.randn((2, input_size[0] * input_size[1], embed_dim)).to(device)
    pm = PatchMerging2D(input_size, merge_size, embed_dim, nn.LayerNorm, patch_mode).to(device)
    y = pm(x)

    expected_shape = (
        2,
        (input_size[0] // merge_size[0]) * (input_size[1] // merge_size[1]),
        embed_dim * merge_size[0] * merge_size[1] // 2,
    )
...
def test_window_shift_2d(device, input_size, shift_size, embed_dim):
    x = torch.randn((2, input_size[0] * input_size[1], embed_dim)).to(device)
    cs = WindowShift2D(input_size, shift_size).to(device)
    csr = WindowShift2D(input_size, shift_size, reverse=True).to(device)
    ...
    assert shifted.device.type == device.type, f"Expected device {device}, got {shifted.device}"


@pytest.mark.parametrize(
    "input_size,window_size,embed_dim,num_heads,rpe_mode",
    [
        ((6, 6), (2, 2), 12, 3, RelativePositionalEmeddingMode.BIAS),
        ((6, 6), (2, 2), 12, 3, RelativePositionalEmeddingMode.CONTEXT),
        ((6, 6), (2, 2), 12, 3, RelativePositionalEmeddingMode.NONE),
        ((8, 8), (4, 4), 64, 8, RelativePositionalEmeddingMode.BIAS),
    ],
)
def test_attention_2d(device, input_size, window_size, embed_dim, num_heads, rpe_mode):
    x = torch.randn((2, input_size[0] * input_size[1], embed_dim)).to(device)
    attn = Attention2D(
        input_size,
        window_size,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SwinTransformerConfig2D(
            ...
            block_window_size=[(4, 4), (4, 4), (4, 4), (4, 4)],
            num_heads=[4, 8, 16, 32],
            mlp_ratio=4.0,
            qkv_bias=True,
            qk_scale=None,
...
def test_swin_transformer_2d(device, config):
    batch = 2
    x = torch.randn((batch, config.in_channels, *config.input_size)).to(device)
    model = SwinTransformer2D(config).to(device)
    ...
    for i, (xo, pws) in enumerate(zip(x_out, config.patch_window_size)):
        ins = tuple(ins[j] // pws[j] for j in range(2))
        expected_shape = (batch, ins[0] * ins[1], dims)
        assert xo.shape == expected_shape, f"Stage {i}: Expected shape {expected_shape}, got {xo.shape}"
        assert xo.device.type == device.type, f"Stage {i}: Expected device {device}, got {xo.device}"
