...
@pytest.mark.parametrize("PatchMerging, dims", [
    (PatchMerging2D, 2),
    (PatchMerging3D, 3)
])
class TestPatchMerging:
    ...
    def test_invalid_input_size(self, PatchMerging, dims, merge_params):
        ...
        batch_size = 2
        if dims == 2:
            x = torch.randn(batch_size, merge_params['in_channels'], 55, 55)
        else:
            x = torch.randn(batch_size, merge_params['in_channels'], 55, 55, 55)
